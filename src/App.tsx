...
const KNOWN_CITIES = [
  "kolkata","bangalore","bengaluru","delhi","mumbai",
...
const getImg = (item: Listing): string => {
  if (item.image_urls) { const f=item.image_urls.split(",")[0].trim(); if (f.startsWith("http")) return f; }
  let h=0; const s=item.post_id||"";
  for (let i=0;i<s.length;i++) h=(h*31+s.charCodeAt(i))&0xffff;
  return IMGS[h%IMGS.length];
};

const FilterPill = ({ label, options, value, onChange }: { label:string; options:string[]; value:string; onChange:(v:string)=>void }) => (
...
const Card = ({ item, onClick, liked, onLike }: { item:Listing; onClick:(i:Listing)=>void; liked:boolean; onLike:(id:string)=>void }) => (
...
export default function App() {
  ...
  const StatusBanner = ()=>{
    if (loadState==="loading") return <div style={{display:"flex",alignItems:"center",gap:"8px",background:"rgba(99,102,241,0.1)",border:"1px solid rgba(99,102,241,0.2)",borderRadius:"9999px",padding:"6px 16px",fontSize:"10px",color:"#818cf8",fontWeight:700}}><Loader size={11} style={{animation:"spin 1s linear infinite"}}/> Connecting to BigQuery\u2026</div>;
    if (loadState==="done")    return <div style={{display:"flex",alignItems:"center",gap:"6px",background:"rgba(16,185,129,0.08)",border:"1px solid rgba(16,185,129,0.2)",borderRadius:"9999px",padding:"4px 14px",fontSize:"10px",fontWeight:700,color:"#10b981"}}>\u2713 {allData.length} live listings across {opts.cities.length} cities</div>;
    if (loadState==="error")   return <div style={{display:"flex",alignItems:"center",gap:"8px",background:"rgba(251,191,36,0.08)",border:"1px solid rgba(251,191,36,0.2)",borderRadius:"9999px",padding:"5px 14px",fontSize:"10px",color:"#fbbf24",fontWeight:700}}><AlertCircle size={11}/> Showing preview \u2014 <button onClick={loadLive} style={{background:"none",border:"none",color:"#fbbf24",cursor:"pointer",textDecoration:"underline",fontSize:"10px",fontWeight:700,padding:0,display:"flex",alignItems:"center",gap:"3px"}}><RefreshCw size={10}/> Retry</button></div>;
    return <div style={{display:"flex",alignItems:"center",gap:"6px",background:"rgba(99,102,241,0.08)",border:"1px solid rgba(99,102,241,0.15)",borderRadius:"9999px",padding:"4px 14px",fontSize:"10px",fontWeight:700,color:"#818cf8"}}>Preview \u00b7 {allData.length} sample listings</div>;
  };
            ...
            <div style={{maxWidth:"680px",width:"100%",textAlign:"center",display:"flex",flexDirection:"column",gap:"22px",alignItems:"center"}}>
              ...
              <div style={{background:"#111",border:"1px solid rgba(255,255,255,0.1)",borderRadius:"9999px",padding:"5px",display:"flex",alignItems:"center",gap:"6px",width:"100%",maxWidth:"580px",boxShadow:"0 20px 60px rgba(0,0,0,0.6)"}}>
                ...
                <input autoFocus
                  ...
                  placeholder="1BHK in Kolkata, fully furnished\u2026"
                ...
                <button onClick={()=>doSearch()} style={{background:"white",color:"black",border:"none",borderRadius:"9999px",padding:"9px 22px",fontSize:"10px",fontWeight:900,textTransform:"uppercase",letterSpacing:"0.1em",cursor:"pointer",whiteSpace:"nowrap"}}>
                  Search
                </button>
              </div>
              ...
              {loadState==="error" && (
                <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:"12px",padding:"12px 16px",fontSize:"11px",color:"#52525b",maxWidth:"480px",lineHeight:1.7}}>
                  \ud83d\udca1 <strong style={{color:"#71717a"}}>For live data:</strong> Open this app outside Claude \u2014 the browser will fetch all listings directly from your BigQuery API.
                </div>
              )}
            </div>
          </div>
          ...
          <div style={{maxWidth:"1200px",margin:"0 auto",padding:"16px 16px 80px"}}>
            <div style={{position:"sticky",top:"62px",zIndex:40,marginBottom:"20px"}}>
              <div style={{background:"rgba(12,12,12,0.96)",backdropFilter:"blur(20px)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:"12px",padding:"5px 10px",display:"flex",alignItems:"center",gap:"7px"}}>
                ...
                <input style={{background:"transparent",border:"none",outline:"none",flex:1,fontSize:"12px",fontWeight:700,color:"white"}}
            ...
            {results.length>0 ? (
              <>
                ...
                {paged.length<results.length && (
                  <div style={{textAlign:"center",marginTop:"28px"}}>
                    <button onClick={()=>setPage(p=>p+1)} style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",color:"#a1a1aa",borderRadius:"9999px",padding:"10px 28px",fontSize:"10px",fontWeight:700,cursor:"pointer",textTransform:"uppercase",letterSpacing:"0.1em"}}>
              ...
              <div style={{textAlign:"center",padding:"80px 0",color:"#3f3f46"}}>
                ...
                <p style={{fontSize:"11px",color:"#27272a"}}>Try different filters or a broader search</p>
              </div>
            )}
          </div>
        )}
