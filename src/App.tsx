...
const parseNLP = (q: string): NLPResult => {
  ...
  if (underMatch) {
    const val = parseInt(underMatch[1]) * (s[(underMatch.index ?? 0) + underMatch[0].length - 1] === 'k' || parseInt(underMatch[1]) < 500 ? 1000 : 1);
    r.maxPrice = val;
  }
...
const EMPTY_FILTERS: FiltersState = {city:'',bhk:'',furnishing:'',preference:'',priceRange:'',minPrice:'',maxPrice:''};
...
export default function App() {
  ...
  const doSearch = useCallback((q = searchQuery, f: FiltersState = filters) => {
    ...
    const out = allData.filter(i => {
      ...
      const minP = merged.minPrice !== '' && merged.minPrice != null ? parseInt(String(merged.minPrice)) : null;
      const maxP = merged.maxPrice !== '' && merged.maxPrice != null ? parseInt(String(merged.maxPrice)) : null;
      if (minP !== null && (i.price ?? 0) < minP) return false;
      if (maxP !== null && (i.price ?? 0) > maxP) return false;
      return true;
    });
